...
public class Solution {
    ...
    public static class DataAdapter implements RowItem{
        ...
        public String getContactFirstName() {
            int start = contact.getName().lastIndexOf(" ");
            return contact.getName().substring(start +1);

        }

        @Override
        public String getContactLastName() {
            int end = contact.getName().lastIndexOf(",");
            return contact.getName().substring(0, end);

        }

        @Override
        public String getDialString() {
            return "callto://"+contact.getPhoneNumber().replaceAll("[\\s\\-()]","");
        }
    }
